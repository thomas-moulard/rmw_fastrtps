...
/**
 * Get the guid that corresponds to the nodeand namespace.
 *
 ...
 */
rmw_ret_t __get_guid_by_name(const rmw_node_t * node, const char * node_name,
    ...
    if (strcmp(node->name, node_name) == 0) {
        ...
        auto guid_node_pair = std::find_if(impl->listener->discovered_names.begin(),
                                           ...
                                           [node_name, &nodes_in_desired_namespace](std::pair<const GUID_t, std::string> pair) {
 ...
 */
void
__accumulate_topics(const LockedObject<TopicCache>& topic_cache,
 ...
 */
rmw_ret_t
__copy_data_to_results(const std::map<std::string, std::set<std::string>> &topics,
        rcutils_allocator_t *allocator,
        bool no_demangle,
        rmw_names_and_types_t *topic_names_and_types)
{
    ...
    if (topics.size() > 0) {
        ...
        if (no_demangle) {
            auto noop = [](const std::__cxx11::string &in) {
                return in;
            };
            demangle_topic = noop;
            demangle_type = noop;
 ...
 */
typedef std::function<const LockedObject<TopicCache>&(CustomParticipantInfo& participant_info)> RetrieveCache;
...
__rmw_get_subscriber_names_and_types_by_node(
        ...
        rmw_names_and_types_t * topic_names_and_types)
{
    RetrieveCache retrieve_sub_cache = [](CustomParticipantInfo& participant_info) -> const LockedObject<TopicCache>& {
        return participant_info.secondarySubListener->topic_cache_;
    };
...
__rmw_get_publisher_names_and_types_by_node(
        ...
        bool no_demangle,
        rmw_names_and_types_t * topic_names_and_types) {
    RetrieveCache retrieve_pub_cache = [](CustomParticipantInfo& participant_info) -> const LockedObject<TopicCache>& {
      return participant_info.secondaryPubListener->topic_cache_;
    };
    return __rmw_get_topic_names_and_types_by_node(identifier, node, allocator, node_name,
